...
export async function generateMetadata({ params }: { params: Promise<{ year: string }> }) {
  ...
  return { title: `Доходы ${year}` };
}

export default async function IncomePage({ params }: { params: Promise<{ year: string }> }) {
  const { year } = await params;
        ...
        beside={
          <Group gap="md" wrap="nowrap">
            <ViewNav
              value="sheet"
              options={[
                { value: 'sheet', label: 'Таблица', href: `/income/${year}` },
                { value: 'list', label: 'Список', href: '/income/list' },
              ]}
            />
            <Group gap="xs" wrap="nowrap">
              <AnchorLink href={`/income/${Number(year) - 1}`} fz="sm">
                ← {Number(year) - 1}
              </AnchorLink>
              <AnchorLink href={`/income/${Number(year) + 1}`} fz="sm">
                {Number(year) + 1} →
